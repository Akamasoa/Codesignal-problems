...
var totalMilliseconds = hours * 60 * 60 * 1000 + minutes * 60 * 1000 + seconds * 1000 + milliseconds;
  var lrcLyricsTime = lrcLyrics.map(function(line) {
    var time = line.match(pattern);
    return parseInt(time[1]) * 60 * 60 * 1000 + parseInt(time[2]) * 60 * 1000 + parseInt(time[3]) * 1000 + parseInt(time[4]);
  });
}
